...
get_friends_rl <- function(name = "CedScherer"){
...
```{r}
users_friends_final <- bind_rows(users_friends, users_friends2) 

users_path <- here::here("data/rstats-tweets/tweets_users_path.rds")

if(!file.exists(users_path)) {
  
  users <- users_friends_final %>%
    distinct(user_id) %>% 
    pull(user_id) %>% 
    sort() %>% 
    ...
    map_df(function(x){
      ...
      rtweet::lookup_users(x)
    
    })

  saveRDS(users, users_path)
  
} else {
  
   users <- readRDS(users_path)
